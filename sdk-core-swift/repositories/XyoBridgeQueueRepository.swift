...
/// A repisority to facilate the persistance of a queue, or a series of weights and hashes. It is optional to implement a caching
/// mechanismmbehind this repo, but it is highly recomended. This repo does not need to persist any data until the
/// commit() function is called. No getter should have a defualt value.
public protocol XyoBridgeQueueRepository {
    
    /// This function is called evey time blocks need to be cleaned from the queue. It is called so that
    /// weights can determine if a hash needs to be removed from the queue.
    /// - Returns: Will return the entire queue, in no particular order. And will return an emepty list if
    /// the queue is empty.
    func getQueue () -> [XyoBridgeQueueItem]
    
    /// This function is not currently called in the XYO core, and is only called by certian unit tests. Although it should be
    /// implemented for futrue use.
    ...
    /// This function is called every time a new bound witness is discocered to add to the queue. It does not matter
    ...
    /// function is called after every sucessfull bridge is made to keep the storage light.
    /// - Parameter hashes: the hashes to remove from the persisted queue.
    func removeQueueItems (hashes : [XyoObjectStructure])
    
    /// This function should get the lowest weight queue items and return them. It should return the number of paramater n, if the
    /// repo does not have n queue items, it should return the entire queue. There is no particular order the result should be in.
    /// This function is called evey time the device is getting ready to bridge.
    ...
    /// This function is called evey time after a sucessfull bridge event to increment their weights by weight += 1.
