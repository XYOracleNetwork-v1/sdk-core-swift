...
public class XyoSecp256k1Signer : XyoSigner {
    ...
    private let secp256k1SignConext : OpaquePointer
    private let secp256k1VerifyConext : OpaquePointer
    private var publicKey : secp256k1_pubkey = secp256k1_pubkey.init()
    
    public init (privateKeyNum : [UInt8]) {
        self.privateKey = privateKeyNum
        self.secp256k1SignConext = secp256k1_context_create(UInt32(SECP256K1_CONTEXT_SIGN))
        self.secp256k1VerifyConext = secp256k1_context_create(UInt32(SECP256K1_CONTEXT_VERIFY))
        
        _ = secp256k1_ec_pubkey_create(secp256k1SignConext, &publicKey, &privateKey)
    }
    ...
    public func getPublicKey () -> XyoObjectStructure {
        ...
        var size = 65
        secp256k1_ec_pubkey_serialize(secp256k1SignConext, &out, &size, &publicKey, UInt32(SECP256K1_EC_UNCOMPRESSED))
        out.remove(at: 0)
    ...
    public func sign (data : [UInt8]) -> XyoObjectStructure {
        ...
        secp256k1_ecdsa_sign(secp256k1SignConext, &sig, hashTo32BytesSha256(data: data), &privateKey, secp256k1_nonce_function_default, &nonce)
        secp256k1_ecdsa_signature_serialize_der(secp256k1SignConext, &out, &length, &sig)
    ...
    private func getRFromD(der : [UInt8]) -> [UInt8] {
        ...
        return buffer.copyRangeOf(from: 3, to: (3 + sizeOfR)).toByteArray()
    }
    ...
    private func getSFromD(der : [UInt8]) -> [UInt8] {
        ...
        let buffer = XyoBuffer(data: der)
        return buffer.copyRangeOf(from: 5 + sizeOfR, to: 5 + sizeOfR + sizeOfS).toByteArray()
    }
